...
public class MetricsControllerImpl implements MetricsController {
    ...
    @Override
    public ResponseEntity<ProgressResponse> getProgress(User user, UUID testId, TestMode mode, LocalDate from, LocalDate to) {
        return ResponseEntity.ok(metricsService.getProgress(user, testId, mode, from, to));
    }
    ...
    @Override
    public ResponseEntity<List<RankingEntryDto>> getRanking(UUID testId, String sortBy, TestMode mode, LocalDate from, LocalDate to) {
        return ResponseEntity.ok(metricsService.getRanking(testId, sortBy, mode, from, to));
    }
}
